...
Domain: Daedalus-ish net: node / edge / eval-node names; still lambda bodies

## High-quality rewrite
Symbolic graph helpers. No SPICE subprocess. Bodies remain one lambda.

## Eval assumption
node ids are ints; edge is a pair; eval-node returns a number.
...
Re-run `python3 scripts/catalog.py --check --projects --project circ-dae`.
