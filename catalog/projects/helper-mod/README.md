...
Domain: local helper + public caller; find the right name

## High-quality rewrite
Each sample rebinds exactly one name. Plants always have ≥2 top-level defines. Summaries: inline-helper, eta-helper, abs-public, id-helper, const-helper, rename-public.

## Eval assumption
Ints; helper may be unused after public rebind.
...
- never rebind two names in one target
...
Re-run `python3 scripts/catalog.py --check --projects --project helper-mod`.
